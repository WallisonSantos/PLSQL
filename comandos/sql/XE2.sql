...
-- Fazendo INSERT com dados de outra tabela: (de TABELA DE CLIENTES para TB CLIENTES)
INSERT INTO TB_CLIENTES
SELECT  CPF, NOME, ENDERECO_1 AS ENDERECO
    , BAIRRO, CIDADE, ESTADO, CEP, DATA_DE_NASCIMENTO AS DATA_NASCIMENTO
    , IDADE, SEXO, LIMITE_DE_CREDITO AS LIMITE_CREDITO, VOLUME_DE_COMPRA, PRIMEIRA_COMPRA FROM TABELA_DE_CLIENTES
    WHERE CPF NOT IN (SELECT CPF FROM TB_CLIENTES);
...
-- ESTA TRIGGER ESTARÁ ASSOCIADA A INCLUSÃO DE DADOS NA TABELA DE ITENS DE NOTAS, TODA VEZ QUE INCLUIR UM ITEM NOVO É CALCULADO:

CREATE TABLE TB_FATURAMENTO
(DATA_VENDA DATE NULL, TOTAL_VENDA FLOAT);

-- CRIANDO OU ALTERANDO, TRIGGER, DEPOIS DE HAVER UM DADO INSERIDO, ATUALIZADO OU DELETADO NA TB ITEM NOTAS:
-- PODENDO SER UTILIZANDO, BEFORE(ANTES), AFTER(DEPOIS) OU INSTEAD OF(INVES DE... IGNORANDO INSERT, UPDATE, DELETE ...):
CREATE OR REPLACE TRIGGER TG_CALCULO_FATURAMENTO
AFTER INSERT OR UPDATE OR DELETE ON TB_ITEM_NOTAS
BEGIN
-- FAZENDO UM CALCULO DA SOMA DO PREÇO DO ITEM * A QUANTIDADE, AGRUPADO POR DATA, GROUP BY:
DELETE FROM  TB_FATURAMENTO;
INSERT INTO TB_FATURAMENTO
SELECT
    TB_NOTAS.DATA_VENDA
    , SUM(TB_ITEM_NOTAS.QUANTIDADE * TB_ITEM_NOTAS.PRECO) AS TOTAL_VENDA
FROM
    TB_NOTAS INNER JOIN TB_ITEM_NOTAS
    ON TB_ITEM_NOTAS.NUMERO = TB_NOTAS.NUMERO
    GROUP BY TB_NOTAS.DATA_VENDA;
END;

INSERT INTO TB_NOTAS (NUMERO, DATA_VENDA, CPF, MATRICULA, IMPOSTO)
VALUES
('0104', TO_DATE('2019-06-06','YYYY-MM-DD'), '3623344710', '235', 0.10);
INSERT INTO TB_ITEM_NOTAS (NUMERO, CODIGO, QUANTIDADE, PRECO)
VALUES
...
UPDATE TB_ITEM_NOTAS SET QUANTIDADE = 3000
WHERE NUMERO = '0104' AND CODIGO = '1000889';

DELETE FROM TB_ITEM_NOTAS
WHERE NUMERO = '0104' AND CODIGO = '1000889';

SELECT * FROM tb_faturamento;
SELECT * FROM tb_item_notas;
